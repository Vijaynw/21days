...
export default function HabitDetailScreen() {
  ...
  const loadHabit = useCallback(async function() {
    ...
    const found = habits.find(function(h) { return h.id === id; });
    if (found) {
      setHabit(found);
  ...
  const handleSaveName = async function() {
    ...
    const updatedHabit = Object.assign({}, habit, { name: trimmed });
  ...
  const toggleEditing = function() {
    if (isEditingName) {
      ...
      setEditedName(habit ? habit.name : '');
  ...
  const getMonthCompletions = function() {
    const year = calendarDate.getFullYear();
    const month = calendarDate.getMonth();
    let full = 0;
    
    habit.completions.forEach(function(dateStr) {
