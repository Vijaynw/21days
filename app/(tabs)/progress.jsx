...
export default function ProgressScreen() {
  ...
  const getTotalCompletions = function() {
    return habits.reduce(function(sum, h) { return sum + h.completions.length; }, 0);
  };

  const getTodayCompletions = function() {
    const today = formatDate(new Date());
    return habits.filter(function(h) { return h.completions.includes(today); }).length;
  };

  const getWeeklyCompletions = function() {
  ...
  const getCompletionRate = function() {
    ...
    const today = formatDate(new Date());
    const completed = habits.filter(function(h) { return h.completions.includes(today); }).length;
    return Math.round((completed / habits.length) * 100);
  };

  const getWeekData = function() {
    const data = [];
    for (let i = 6; i >= 0; i--) {
      const date = new Date();
      date.setDate(date.getDate() - i);
      const dateStr = formatDate(date);
      const completed = habits.filter(function(h) { return h.completions.includes(dateStr); }).length;
      data.push({
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.chartCard}>
          ...
          <View style={styles.chartContainer}>
            {weekData.map(function(item, index) {
              return (
