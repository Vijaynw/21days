/**
 * Calendar/Explore Screen (JavaScript)
 */
...
export default function CalendarScreen() {
  ...
  const loadHabits = useCallback(async function() {
  ...
  const isDateCompleted = function(day) {
    ...
    const habit = habits.find(function(h) { return h.id === selectedHabit; });
  ...
  const selectedHabitData = habits.find(function(h) { return h.id === selectedHabit; });
      ...
      {habits.length === 0 ? (
        ...
        <ScrollView style={styles.content}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {habits.map(function(habit) {
                return (
                  <TouchableOpacity
                    key={habit.id}
                    onPress={function() { setSelectedHabit(habit.id); }}
                    style={[
                      styles.habitChip,
                      {
                        backgroundColor:
          ...
          <View style={styles.monthNav}>
            <TouchableOpacity onPress={function() { changeMonth(-1); }} style={styles.navButton}>
              <IconSymbol name="chevron.left" size={24} color={selectedHabitData ? selectedHabitData.color : Colors[colorScheme || 'light'].tint} />
            </TouchableOpacity>
            <ThemedText type="subtitle" style={styles.monthText}>
              {MONTHS[currentDate.getMonth()]} {currentDate.getFullYear()}
            </ThemedText>
            <TouchableOpacity onPress={function() { changeMonth(1); }} style={styles.navButton}>
              <IconSymbol name="chevron.right" size={24} color={selectedHabitData ? selectedHabitData.color : Colors[colorScheme || 'light'].tint} />
            </TouchableOpacity>
          </View>
                    ...
                    {day !== null && (
                      <View
                        style={[
                          styles.dayCircle,
                          isDateCompleted(day) && {
                            backgroundColor: selectedHabitData ? selectedHabitData.color : '#ccc',
                          },
                        ]}>
                        <ThemedText
                          style={[
